...
''' test
Usage:
    $ spark-submit test.py hdfs:/user/zj444 10 0.1 0.05
'''
...
def main(spark, rank, regParam, path, fraction):
    ...
    print("Loading data...")
    testing = spark.read.parquet("{}/data/processed/testing_{}.parquet".format(path, fraction))
    ...
    groundtruth = testing.groupby("user_id").agg(F.collect_set("book_id").alias('groundtruth'))
...
if __name__ == "__main__":
    memory = "8g"
    spark = SparkSession.builder.appName("testing").master('yarn').config("spark.executor.memory", memory).config("spark.driver.memory", memory).getOrCreate()
    path = sys.argv[1]
    rank = int(sys.argv[2])
